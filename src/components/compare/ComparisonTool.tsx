    ...
    <section id="compare" className="relative bg-ink px-6 py-28 md:px-14">
      ...
      <div className="mx-auto max-w-6xl overflow-x-auto">
        <table className="w-full min-w-[640px] border-collapse text-left">
          <thead>
            <tr>
              <th className="w-40 border-b border-line pb-4 pr-4 text-xs uppercase tracking-[0.2em] text-gold-dim">
                Criteria
              </th>
              {selectedCountries.map((c) => (
                <th
                  key={c.slug}
                  className="border-b border-line px-4 pb-4 font-display text-base italic text-parchment md:text-lg"
                >
                  {c.name}
                </th>
              ))}
              {selectedCountries.length === 0 && (
                <th className="border-b border-line px-4 pb-4 text-sm text-parchment-dim">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} className="border-b border-line/60">
                <td className="py-4 pr-4 text-xs uppercase tracking-[0.15em] text-gold-dim">
                  {row.label}
                </td>
                {selectedCountries.map((c) => (
                  <td
                    key={c.slug}
                    className={`px-4 py-4 text-xs text-parchment-dim md:text-sm ${
                      row.label === "Investment" ? "font-mono-figures text-gold-bright" : ""
                    }`}
                  >
                    {row.render(c)}
                  </td>
